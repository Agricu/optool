...
// 核心修改1：将CC_MD5_CTX替换为CC_SHA256_CTX
- (void)updateHash:(CC_SHA256_CTX *)sha256
{
    ...
    // 核心修改2：CC_MD5_Update替换为CC_SHA256_Update
    CC_SHA256_Update(sha256, (const void *)&classHash, sizeof(NSUInteger));
    
    for (NSString * s in _switches) {
        NSUInteger hash = [xpmargs_expandSwitch(s) hash];
        CC_SHA256_Update(sha256, (const void *)&hash, sizeof(NSUInteger));
    }
